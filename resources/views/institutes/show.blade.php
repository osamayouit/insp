...
<body class="bg-gray-50 p-6">
    ...
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-2xl shadow-md border-l-8 border-green-500">
        <div class="flex items-center space-x-4 space-x-reverse">
            @if($institute->image)
                <img src="{{ asset('storage/' . $institute->image) }}" alt="صورة الجامعة" class="w-32 h-32 object-cover rounded-xl border shadow">
            @endif
            <div>
                <h1 class="text-3xl font-bold text-green-700 mb-2">تفاصيل الجامعة والتخصص</h1>
                <p class="text-lg text-gray-700"><strong>اسم الجامعة:</strong> {{ $institute->name }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="mt-8 max-w-4xl mx-auto bg-white p-6 rounded-2xl shadow-md border border-gray-200">
        ...
        <form action="{{ route('messages.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @csrf
            <input type="hidden" name="institute_name" value="{{ $institute->name }}">
            ...
            <div>
                ...
                <select name="course_name" required class="w-full select2">
            ...
            <div>
                ...
                <div>
                    ...
                    <input type="file" name="institute_certificate"
                        class="w-full rounded-xl border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 text-lg p-4">
                </div>
